...
**状态：✅ 已通过评审，但被改进了**（@Genicayyy，09-12 22:23）。
最终形态以 [`spec.md` §4](spec.md) 为准 —— 她把统一的蕴含校验改成了**按声称类型分层**，
并加了 `uncertain` 第三态。改得比原提案好，理由见下方"评审结论"。

**原提案：** 影响 [`spec.md`](spec.md) 第 4 节（架构）和第 6 节（评测）。
...
@Genicayyy 通过了这个提案，但做了三处改进，**都比原提案好**：

**1. 重新定位** —— 这是「模型输出进入风险评分前的安全门」，不是升级判别标准本身。

**2. 按声称类型分层，而不是统一做 LLM 蕴含校验**

| 类型 | 校验 |
| --- | --- |
| 硬事实（承诺、退款诉求、监管表述） | 逐字引文 + **确定性字符串校验** |
| 语义判断（未解决诉求、信任下降、情绪趋势） | 多段证据 + 蕴含校验，**主链路稳定后再做** |
| 声学情绪 | 保留音频时间戳 |

**关键洞察**：风险最高的那类声称——凭空捏造一条承诺去冤枉一个真人——
**恰好是最容易用字符串精确验证的**。所以最重要的安全保证几乎零成本，
昂贵的 LLM 校验留给模糊的那类且可延后。

**3. 三态而非二态** —— 证据不足标记 `uncertain`，不进入评分但在界面显示。
系统说"我不确定"比系统猜一个更可信。

她还补了四个风险信号：**无人负责 · 答复矛盾 · 问题影响 · 信任变化**，
并重申**情绪是弱信号，不能单独触发高风险**。

以及实现优先级：**主链路端到端 > 引文校验 + 核心业务信号 > 蕴含校验 > 声学情绪**。
