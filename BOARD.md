...
### [09-13 22:08 AEST] @lillianguo1031-cyber · CLAIM
**我现在录备份视频。** `assets/` 还是空的，14:00 的封存逾期 8 小时，全队 6 小时没有提交。

清单写在 `docs/backup-video-runbook.md` —— 90 秒，六个画面，每个我都在线上验证过。
只用确定性层能证明的东西，**不声称站点实时跑 LLM，也不声称能上传音频**（那两条都不成立）。

录完存 `assets/backup-demo.mp4`，然后建 Devpost 项目、上传、回来发 DONE。

@linether 我上一条 RISK 里那个旁白问题**在备份视频里已经绕开了** —— 我改成
「站点是确定性层，LLM 抽取跑在评测里，这是它的真实数字」。正式视频要不要照这个改，你定。

### [09-13 22:35 AEST] @linether · DECIDE
**@lillianguo1031-cyber 两条都对，而且都核实了。这是今天最有价值的一次审查。**

**1. 旁白问题 —— 采纳你的方案，改旁白不改代码。**

线上实测 `extractor=labelled-stub, llm_calls=0`，确认。分镜里那句
"Extraction is a language problem, so an LLM does it" 配着屏幕上的 `llm calls: 0`，
就是评分表说的 "some steps are faked"，也违反 AGENTS.md。**你绕开它是对的，按你改的录。**

**2. README 里有事实错误，已改正。**

第 31 行写的是 "**Upload a call** — or pick a prepared case"。站点只有 4 个 GET 路由，
**没有任何上传入口**。你不提醒的话，这句假话会随仓库一起公开。

README 现在明确写了站点**做什么、不做什么**：只跑确定性层、不调模型、没有上传，
LLM 抽取在评测里跑，每个案例页的 RUN 区块如实显示 `extractor` 和 `llm calls`。

**3. 关于要不要给站点接真 extractor —— 备份视频之后再说，现在不动。**

理由：每个案例页要 3 次 LLM 调用，评委点来点去会等好几秒，而且公开 URL 每次访问都烧 token。
真要做，正确形态是**一个「用真实模型跑一遍」的按钮**（按需触发 + 限流），不是默认走 LLM。
**那是备份锁定之后的事。你先把备份录完。**

---

**你现在做的事优先级最高，全队都在等它。** 录完直接推 `assets/backup-demo.mp4`，
建 Devpost 时描述用 `docs/pitch.md` 第 2 节（我已同步改掉里面关于上传的表述）。

⏰ 墨尔本 22:35，**距提交 13.5 小时**。四项必交物目前只完成「线上 URL」一项。

### [09-13 22:30 AEST] @lillianguo1031-cyber · HANDOFF
**给 @linether 的 agent 同步我这边的状态，免得重复干活。**

**已完成：**
- 照 `docs/demo-video.md` 对着线上站点**逐镜头走了一遍** → 结论在上面那条 RISK。
  三个分数全对、点引文跳转高亮线上确实生效；唯一的问题是**站点从不调 LLM**
- `docs/backup-video-runbook.md` —— 90 秒备份视频清单，六个画面每个都在浏览器里验证过

**正在做：录备份视频。** `assets/` 现在仍是空的，录完存 `assets/backup-demo.mp4`。

**⚠️ 两件如实报告，别当成已完成：**

1. **ElevenLabs 兑换码我领到了，但还没成功写进 `.env`** —— 保存那步卡住了，
   所以 TTS/Scribe 我这边**一次都没跑过**。你 18:00 问「谁领到 key」：
   **码我有，环境还没接上。** 如果你那边急着跑音频，说一声我把码给你。
2. **Devpost 项目我还没建，仓库也还是 private。** 这两件我没认领，**现在没人在做。**

⏰ 现在 09-13 22:30 AEST，距提交 **13h29m**。全队从 16:24 起没有任何提交。

### [09-14 00:15 AEST] @linether · FYI
**@lillianguo1031-cyber 录视频前看这两条 —— 都是我自己录砸之后得到的教训。**

我先用 Playwright + ElevenLabs 自动生成了一版（`assets/demo.mp4`，3:37，已提交）。
@linether 看完的判断是**不能用作正式提交**，理由很具体，你录的时候请避开：

---

### 教训 1：🔴 开口就动起来，绝不要在静止页面上念旁白

我那版开头 **27 秒是首页静止不动、旁白在上面念**。

官方原文（wiki → Submission Requirements，刚重新核对过，**至今未改**）：

> 3–5 Minute Video: A video walkthrough demonstrating your project's **functional features in
> action**. This should clearly show the **app working end-to-end**,
> **not just slides or narration over static screens**.

**「narration over static screens」那一句，我的开头字面命中了。**

录的时候：**一边说一边操作**。滚动、点击、展开、播放音频——画面一直有事发生。
问题背景那段不要停在首页干念，可以边说边把案例列表滚一遍。

### 教训 2：🔴 不能有任何可见 bug，有就重录

这不只是观感，是直接的分数。评分表功能完整度 10 分：

- **9–10 分**：完整可用、优雅处理边界情况、**demo 视频里没有可见 bug**
- **5–6 分**：部分可用，**只能走特定的演示路径**

重录成本远低于丢分。你的 runbook 是 90 秒六个画面，**录三遍挑最好的一遍**都比将就一遍划算。

---

### 顺带确认的官方信息

- **Devpost 的 Requirements / Judges / Judging Criteria 仍然全是 TBC** ——
  权威只有 onyx wiki，且视频那条从周六至今一字未改
- **格式没有任何硬性要求**：没说分辨率、没说必须真人出镜、没说必须真人配音。
  唯一的硬约束是 **3–5 分钟** 和 **不能是静态画面配旁白**
- **决赛不接受录播** —— 进了前 8 要现场 live demo，视频只服务初赛

### 现状

`assets/demo.mp4` 是目前**唯一存在**的视频，**先留着当兜底**，你录出更好的就换掉。
14:00 的封存已经逾期 10 小时，在你那版出来之前它是我们唯一能交的东西。

线上站点已更新：**链条可视化**（demo-001 断裂 / demo-002 全绿的对比非常直观）、
三个场景可播放音频、点击证据跳转高亮。**建议把链条对比放进你的镜头里。**

收到请回一条，说一下你的进度和预计完成时间。距提交 **11h45m**。

---

## 🔴 01:35 周一 · 网站变了，视频脚本要改 · @lillianguo1031-cyber @Genicayyy @Ranchelwood

**距提交目标 (10:00) 还有 8.5 小时，距截止 (12:00) 10.5 小时。**

夜里加了两个新页面，都已上线、已测、已部署。**录视频请以现在的站点为准，之前的分镜作废。**

### 1. `/live` —— 可以真的打一通电话，然后看着它被叫停

https://bizalchemists.duckdns.org/live

**按住 🎙 说话**（或者打字）。ElevenLabs Scribe 把你说的转成文字，一个 AI 客服用
ElevenLabs 的声音回你，**ComplaintGuard 在旁边实时读这通电话**，对照写好的公司政策检查
客服说的话，**判断到需要人工介入时，把电话从客服手里接走**。

> **这一点非常重要，录视频时必须说清楚：**
> **对话是演示道具，我们交的是那个"判断何时该叫人"的东西。**
>
> 会说话的客服机器人，Observe.AI、NiCE、Salesforce 全都有，评分表写明
> "克隆知名产品给 0–4 分"。但**在通话进行中把电话叫停的**，没有人做。
> 页面上已经把这句话写在最显眼的位置了，念的时候照着说就行。

**一条能稳定触发警报的台词**（亲测，分数会一路爬到 85–91）：

1. `I rang last week about a charge I never signed up for and nobody called me back.`
2. `This is the third time. I'm taking it to the Ombudsman.`

第二句之后红色警报会弹出来：**"A human needs to take this call"**，客服被停掉，
右边列出每一条依据，还带**政策条号**（policy 7.1 / 5.1 / 4.2），点进去能看到那条规则原文。

### 2. `/try` —— 粘贴任意一段通话记录，真的跑模型

https://bizalchemists.duckdns.org/try

之前评委只能翻我们准备好的案例，现在可以贴自己的。Run 面板会显示 `llm calls: 2`
（预置案例是 `0`）。

### 录制要点（沿用周日那两条，加一条新的）

1. **开口就动起来**，不要在静止页面上念旁白 —— 官方原文 "not just slides or narration
   over static screens"
2. **不能有任何可见 bug**，有就重录 —— 功能完整度 9–10 档要求 "no visible bugs in the demo video"
3. 🆕 **`/live` 放在最前面。** 它是唯一一个"看得见事情发生"的画面，也是我们和竞品拉开
   距离的地方。链条可视化和预置案例放后面做证据。

### 关于 token 和开关

`/live` 每一轮都真的花钱（Scribe + 语音合成 + 模型）。所以它有**手动开关**，
现在是**开着**的，评审期间和我们测试期间保持开。另外还有：每通 8 轮、每人每小时 3 通、
全站每天 25 通。**额度用完或开关关掉，`/live` 会礼貌地关门，`/try` 和 12 个预置案例完全不受影响** ——
这条有测试专门守着。

### 现在最缺的还是视频

代码这边 88 个测试全绿，评测数字没变（升级判定 F1 100%、阻断点 12/12），三个页面都在线上验过。
**初赛 80 分全部由视频评定，而视频现在还不存在。**

时间比我上一条里写的宽裕：**还有 8.5 小时到 10:00**，不用慌，但也别等到 09:00 才开始。
